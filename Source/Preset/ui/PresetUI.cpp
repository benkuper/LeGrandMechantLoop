...
void PresetUI::mouseEnter(const MouseEvent& e)
{
    ...
    if (e.eventComponent == this || e.eventComponent == &itemLabel)
    {
        ...
        for (auto& linked : inspectable->linkedInspectables)
        {
            ...
            {
                if(item->enabledMap.contains(c) && !item->enabledMap[c]) continue;
...
void PresetEditor::buildValuesCC()
{
    ...
    while (it.next())
    {
        ...
        c->canBeDisabledByUser = true;
        if (preset->enabledMap.contains(oc)) c->setEnabled(preset->enabledMap[oc]);
...
void PresetEditor::newMessage(const Controllable::ControllableEvent& e)
{
    if (e.type == Controllable::ControllableEvent::STATE_CHANGED)
    {
        if (controllableMap.contains(e.item))
        {
            preset->enabledMap.set(controllableMap[e.item], e.item->enabled);
        }
    }
}
...
void PresetEditor::newMessage(const Parameter::ParameterEvent& e)
{
    ...
    {
        if (controllableMap.contains(e.parameter))
        {
